...
import com.caoping.cloud.ui.GroupAddEmpActivity;
...
public class GroupDetailsActivity extends BaseActivity implements OnClickListener {
	...
	List<Member> members = new ArrayList<Member>();//用户
	@Override
...
//		idLayout.setVisibility(View.VISIBLE);
		...
		List<String> membershx =  group.getMembers();//获得草坪云用户列表
		String hxUserNames = "";
	...
	private class GridAdapter extends ArrayAdapter<String> {
		...
		public GridAdapter(Context context, int textViewResourceId, List<Member> objects) {
...
//				}
				...
				if(members.size() > position){
					final Member emp = members.get(position);
					convertView.setVisibility(View.VISIBLE);
...
//				Emp emp= DBHelper.getInstance(GroupDetailsActivity.this).getEmpById(username);
					if(emp != null){
						...
						imageLoader.displayImage(emp.getEmpCover(), holder.imageView, CaopingCloudApplication.txOptions, animateFirstListener);
					}else{
						EaseUserUtils.setUserNick("暂无", holder.textView);
//					imageLoader.displayImage(emp.getMm_emp_cover(), holder.imageView, CaopingCloudApplication.txOptions, animateFirstListener);
//					EaseUserUtils.setUserAvatar(getContext(), username, holder.imageView);
					}
					...
					button.setOnClickListener(new OnClickListener() {
						...
						public void onClick(View v) {
							if (isInDeleteMode) {
								// 如果是删除自己，return
								if (EMClient.getInstance().getCurrentUser().equals(emp.getEmpName())) {
								...
								EMLog.d("group", "remove user from group:" + emp.getEmpName());
								deleteMembersFromGroup(emp.getEmpName());
							} else {
						...
						public boolean onLongClick(View v) {
							...
							if (group.getOwner().equals(EMClient.getInstance().getCurrentUser())) {
								new EaseAlertDialog(GroupDetailsActivity.this, null, st15, null, new AlertDialogUser() {
									...
									public void onResult(boolean confirmed, Bundle bundle) {
										if(confirmed){
											addUserToBlackList(emp.getEmpName());
										}
									}
								}, true).show();
